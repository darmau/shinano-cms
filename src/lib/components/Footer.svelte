<script lang="ts">
	const links = [
		{ title: 'About', url: 'https://darmau.design' },
		{ title: 'Docs', url: 'https://darmau.design' },
		{ title: 'Themes', url: 'https://darmau.design' }
	];
	const currentYear = new Date().getFullYear();
</script>

<div class="mx-auto max-w-7xl overflow-hidden px-6 py-8 lg:px-8">
	<nav class="columns-2 flex justify-center gap-8" aria-label="Footer">
		{#each links as link}
			<a href={link.url} target="_blank" class="text-sm leading-6 text-gray-600 hover:text-gray-900"
				>{link.title}</a
			>
		{/each}
	</nav>
