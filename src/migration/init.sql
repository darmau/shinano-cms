...
CREATE TABLE
  article (
    ...
    "reactions" JSONB DEFAULT '{"like":0,"love":0,"haha":0,"wow":0,"sad":0,"think":0}'::jsonb,
    FOREIGN KEY ("lang") REFERENCES language ("id") ON UPDATE CASCADE ON DELETE RESTRICT,
    FOREIGN KEY ("cover") REFERENCES image ("id") ON UPDATE CASCADE ON DELETE SET NULL,
...
CREATE TABLE
  thought (
    ...
    "content_text" TEXT,
    "location" TEXT,
    "push_to_gallery" BOOLEAN DEFAULT false,
    "created_at" TIMESTAMPTZ DEFAULT NOW(),
    "content_json" JSON,
    "content_html" TEXT,
    "page_view" INT DEFAULT 0,
...
CREATE INDEX ON thought ("push_to_gallery");
...
DECLARE
    new_reactions JSONB;
BEGIN
    -- 验证 reaction_type 是否有效
    IF reaction_type NOT IN ('like', 'love', 'haha', 'wow', 'sad', 'think') THEN
    ...
    ELSIF content_type = 'photo' THEN
        ...
        WHERE id = content_id
        RETURNING reactions INTO new_reactions;
    ELSIF content_type = 'thought' THEN
        UPDATE public.thought
        SET reactions = jsonb_set(
            reactions,
            ARRAY[reaction_type],
    ...
    IF new_reactions IS NULL THEN
        RAISE EXCEPTION '% with id % not found', content_type, content_id;
    END IF;
...
GRANT EXECUTE ON FUNCTION public.add_reaction(TEXT, BIGINT, TEXT) TO authenticated, anon;
